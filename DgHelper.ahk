...
if (FullSourceText <> "" AND InStr(FullSourceText, "AID:") )
{
	...
	IfWinExist, Egenskaper för källhänvisning
	{
		...
		if (refPageNumber = "") {
			ControlSend, TComboBox3, {PGUP}, Egenskaper för källhänvisning ; Ställ valet "Prefix" på översta valet (inget) listan, när källan saknar sidnummer. Vi skriver då in "AD: " (ArkivDigital) i sidhänvigningen istället.
			ControlSetText, TEdit2, AD: %refADImageNumber1%, Egenskaper för källhänvisning	; Hänvisningstext (Sidnummer). Regex-matchgrupp 1 från refPageNumber.
		} else {
			ControlSend, TComboBox3, {PGUP}{DOWN}{DOWN}, Egenskaper för källhänvisning ; Ställ valet "Prefix" på tredje valet i listan ("p" för pagina)
